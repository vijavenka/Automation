...
public class ContractValidator {
    ...
    public void validateResponseWithContract(String contract, ValidatableResponse validatableResponse, int status) {
        if (status == 404 || status == 400 || status == 500 || status == 503)
            contract = "error-response-schema.json";

        validateResponseWithContract(contract, validatableResponse);
    }


    @Deprecated
    public void validateResponseWithContract(String contract, String response) {
        if (contract == null || contract.isEmpty()) {
            assertThat(response, isEmptyOrNullString());
            return;
        }
