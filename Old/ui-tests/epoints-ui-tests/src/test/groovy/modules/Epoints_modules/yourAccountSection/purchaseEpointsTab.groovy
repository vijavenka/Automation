 ...
 */
class purchaseEpointsTab extends Module{
    static content = {
        //screen 1
        header{ $('h1') }
        ...
        loaderIcon(required: false){ $('.u-loader') }

        //screen 2
        buyEpointsFormTextSecondScreen{ $('div', text: 'Check the details of you order. Press confirm button to open online payment.') }
    ...
    def clickChosenBuyEpointsDDLOption(number){ waitFor{ buyEpointsFormDDLOption[number].isDisplayed() }; buyEpointsFormDDLOption[number].click() }
    def clickReviewOrderButton(){ waitFor{ buyEpointsReviewOrderButton.isDisplayed() }; buyEpointsReviewOrderButton.click() }
    def clickCancelButton(){ waitFor{ buyEpointsCancelButton.isDisplayed() }; buyEpointsCancelButton.click() }
    ...
    def clickSpendButtonOnSummaryPage(){ waitFor{ spendEpointsButton.isDisplayed() }; spendEpointsButton.click() }
    def clickGoToProfileButtonOnSummaryPage(){ waitFor{ goToProfileButton.isDisplayed() }; goToProfileButton.click() }
}