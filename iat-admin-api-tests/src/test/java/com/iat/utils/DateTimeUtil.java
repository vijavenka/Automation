...
public class DateTimeUtil {
    ...
    public static Date adjustDateByWeeks(Date date, int weeks) {
        return adjustDate(date, Calendar.WEEK_OF_YEAR, weeks);
    }
    ...
    public interface Format {
        ...
        String yyyyMMddTHHmmssSSS = "yyyy-MM-dd'T'HH:mm:ss.SSS";
        String ddMMyyyy_HHmmZ = "dd-MM-yyyy HH:mm Z";
    }

    public static List<String> getDatesFromParams(String params, String format) {
        if (params.contains(","))
            return asList(params.split(","));
        ...
        switch (params.toUpperCase()) {
            case "LAST_YEAR":
                dateTo = new Date();
                dateFrom = adjustDateByYears(dateTo, -1);
                break;
            case "LAST_MONTH":
                dateTo = new Date();
                dateFrom = adjustDateByMonths(dateTo, -1);
                break;
            case "LAST_WEEK":
                dateTo = new Date();
                dateFrom = adjustDateByDays(dateTo, -7);
                break;
            case "EMPTY_RANGE":
                break;
            case "INVALID_RANGE":
                dateFromString = "10.12.2017";
                dateToString = "10.12.2018";
                break;
            default:
                assertThat("Used data range is invalid!", params, is(isOneOf("LAST_YEAR", "LAST_MONTH", "EMPTY_RANGE", "INVALID_RANGE")));
        }

        if (dateFrom != null) dateFromString = convertToString(dateFrom, format, true);
        if (dateTo != null) dateToString = convertToString(dateTo, format, true);

        return asList(dateFromString, dateToString);
    }
}