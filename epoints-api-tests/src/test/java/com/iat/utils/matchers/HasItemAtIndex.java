...
public final class HasItemAtIndex<X> extends TypeSafeDiagnosingMatcher<Iterable<X>> {
    ...
    public static <T> Matcher<Iterable<T>> hasFirstItem(Matcher<T> item) {
        return new HasItemAtIndex<>(0, item);
    }

    @Factory
    public static <T> Matcher<Iterable<T>> hasFirstItem(T value) {
        return hasFirstItem(equalTo(value));
    }
    ...
    protected boolean matchesSafely(Iterable<X> iterable, Description mismatchDescription) {
        ...
        if (list.isEmpty()) {
            ...
            return false;
        } else if (index != null && (index > list.size() - 1 || index < -list.size())) {
    ...
    public void describeTo(Description description) {
        if (index == null)
            ...
            description.appendText(format("iterable with item at index %d: ", index));
