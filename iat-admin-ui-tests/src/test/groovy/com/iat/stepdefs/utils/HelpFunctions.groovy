...
class HelpFunctions {
    ...
    void clearCookiesAndLocalStorage() {
        ...
        waitSomeTime(Config.waitMedium)
    }
    ...
    Long parseDateFromStringToMiliseconds(String date, String format) {
    ...
    String returnRandomString(int len) {
    ...
    def scrolPageUpDown(String direction) {
        waitSomeTime(Config.waitMedium)
        if (direction == 'down')
            ((JavascriptExecutor) browser.getDriver()).executeScript("window.scrollBy(0,1250);", "")
        else
            ((JavascriptExecutor) browser.getDriver()).executeScript("window.scrollBy(0,-1250);", "")
    }
}